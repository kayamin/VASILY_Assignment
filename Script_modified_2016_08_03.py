...
## アイテム i の評価への 他のアイテムの評価の 寄与の大きさ を求めユーザー u のアイテム i の評価を，ユーザー u の他のアイテムへの既知の評価の線形結合で推定する．
...
f = open('./MovieLens_100K/ml-100k/u.data','r');
data = f.readlines();
f.close();
...
## 各ユーザー，アイテム毎に推定した評価値を基にレコメンドするアイテムを決定
## 推定値が前処理で求めたユーザーごとの標準偏差よりも大きければレコメンドする

rec = np.zeros([r_num,c_num])
temp = u_rate_est

for i in range(r_num):

    exist_ind = nonzero_ui_mat[1][nonzero_ui_mat[0] == i]
    temp[i][exist_ind] = np.nan
    cand_ind = temp[i,:] > u_var[0,i]
    rec[i,:] = cand_ind * 1

rec_ind = np.nonzero(rec)

rec_item =[0 for i in range(r_num)]

for i in range(r_num):

    ind = rec_ind[0] == i
    rec_item[i] = rec_ind[1][ind]

f = open ('./reccomendation.csv','w') #レコメンド内容を保存した csv を作成 各行が各ユーザーにレコメンドする映画番号（列番号+1）のリストになる，
csvWriter = csv.writer(f)

for i in range (len(rec_item)):
    csvWriter.writerow(rec_item[i]+1)
